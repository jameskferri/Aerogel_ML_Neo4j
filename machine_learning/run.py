...
def train_model(base_df, y_column, material_col_name, aerogel_type, num_of_trials,
                start_test_split, end_test_split, validation_percent,
                seed, working_dir, save_model, zip_dir=True):

    material_col = base_df[material_col_name]
    base_df = base_df.drop(columns=[material_col_name])

    base_df = featurize(base_df, paper_id_column=None, bit_size=128)

    # Spilt up data
    test_df = base_df.iloc[start_test_split:end_test_split]
    ...
    if save_model:
        estimator = build_estimator(best_params)
        estimator.fit(train_features, train_target, epochs=best_params["epochs"])
        estimator.save(f'{run_name}_model')

    zip_run_name_files(run_name, working_dir, zip_dir)

    sleep(1)


def run_params(base_df, aerogel_type, seed, y_column, material_col, num_of_trials, train_percent, validation_percent, working_dir,
               save_model=False):
    ...
    for i, section in enumerate(sections):
        ...
        train_model(base_df, y_column, material_col, aerogel_type, num_of_trials,
                    start_split, end_split, validation_percent,
                    seed, working_dir, save_model)
